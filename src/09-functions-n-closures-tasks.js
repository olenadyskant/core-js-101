...
function getComposition(f, g) {
  return (...x) => f.call(null, g.apply(null, [...x]));
}
...
function getPolynom(...args) {
  ...
  return (x) => coeffs.reduce((acc, coeff, exponent) => acc + coeff * x ** exponent, 0);
}
...
function logger(func, logFunc) {
  return (...args) => {
    const params = JSON.stringify(args).replace(/^\[|]$/g, '');
...
function getIdGeneratorFunction(startFrom) {
  let countID = startFrom;
  return () => {
    const nextID = countID;
    countID += 1;
    return nextID;
  };
}
